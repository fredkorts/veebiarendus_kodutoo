  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($errors as $code => $desc) {
      if (\Drupal::config('system.site')->get("page.$code") != "/customerror/{$code}") {
        drupal_set_message($this->t('Error reporting is not set for error !error. Please ensure that the default !error page is set to be /customerror/!error on the !link.', array('!error' => $code, '!link' => Link::createFromRoute($this->t('Site information settings page'), 'system.site_information_settings')->toString())), 'error', FALSE);
      }
    }
    foreach ($errors as $code => $desc) {
      $form[$code] = array(
        ...
        '#collapsible' => TRUE,
        '#collapsed' => FALSE,
        '#title' => $this->t('Code !code settings', array('!code' => $code)),
        '#tree' => TRUE,
      );
